...
# Run a VideoAgent official-style (prompt/step logic) caption retrieval baseline on NExT-QA val
# using Qwen2.5-VL-7B via local vLLM (4 shards, TP=1 per GPU), in a tmux session.
...
SESSION="${SESSION:-nextqa_videoagent_officialstyle_caption_val_tp1}"
DATE="${DATE:-$(date +%F)}"
...
OUT_DIR="${OUT_DIR:-outputs/${DATE}/nextqa_caption_compare_qwen2p5vl7b/videoagent_officialstyle_caption_val_tp1}"
mkdir -p "${OUT_DIR}"
...
WANDB_GROUP="${WANDB_GROUP:-nextqa_val_videoagent_officialstyle_caption_tp1}"
...
mkcmd() {
  ...
  local wandb_name="videoagent_officialstyle_caption_val_tp1_s${shard}"
...
" ${PY} examples/videoagent/eval_nextqa_videoagent_officialstyle_caption_vllm.py"\
...
cmd0="$(mkcmd 0 0 18300)"
cmd1="$(mkcmd 1 1 18301)"
cmd2="$(mkcmd 2 2 18302)"
cmd3="$(mkcmd 3 3 18303)"
