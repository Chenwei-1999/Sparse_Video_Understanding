...
# Run REVISE (frames + captions) on NExT-QA val with 4 shards (TP=1 per GPU),
# in a tmux session so the job survives disconnects.
#
# Usage:
...
#   WANDB_GROUP=nextqa_val_revise_plus_caption_tp1
...
WANDB_GROUP="${WANDB_GROUP:-nextqa_val_revise_plus_caption_tp1}"
...
mkcmd() {
  ...
  local wandb_name="revise_plus_caption_val_tp1_s${shard}"
...
" --use-candidate-frames --use-candidate-frame-ids --require-candidate-frames"\
" --captions-dir '${CAPTIONS_DIR}' --caption-include both --caption-max-chars 200"\
...
" --resume-from-log --force-final-answer"\
" --use-wandb --wandb-project '${WANDB_PROJECT}' --wandb-entity '${WANDB_ENTITY}'"\
" --wandb-group '${WANDB_GROUP}' --wandb-name '${wandb_name}'"\
") 2>&1 | tee -a '${run_log}'"
}

cmd0="$(mkcmd 0 0 18100)"
cmd1="$(mkcmd 1 1 18101)"
cmd2="$(mkcmd 2 2 18102)"
cmd3="$(mkcmd 3 3 18103)"
