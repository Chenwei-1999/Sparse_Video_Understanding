...
# Plug-and-play eval on lmms-lab/Video-MME (test split) using 4 shards (1 GPU each, TP=1).
...
OUT_DIR="${OUT_DIR:-outputs/2026-01-24/videomme_test_pnp_qwen2p5vl7b_think_ids_tp1}"
LOG_DIR="${LOG_DIR:-debug_runs}"
PROMPT_LOG_JSONL="${PROMPT_LOG_JSONL:-debug_prompt_logs/videomme_test_pnp_qwen2p5vl7b_think_ids_tp1.jsonl}"
SUMMARY_JSON="${SUMMARY_JSON:-${OUT_DIR}/summary.json}"
PIDS_FILE="${PIDS_FILE:-${LOG_DIR}/videomme_test_pnp_qwen2p5vl7b_think_ids_tp1_pids.txt}"
...
WANDB_GROUP="${WANDB_GROUP:-videomme_test_pnp_qwen2p5vl7b_think_ids_tp1}"
...
for shard in 0 1 2 3; do
  gpu="${shard}"
  port="$((19000 + shard))"
  log="${LOG_DIR}/videomme_test_pnp_qwen2p5vl7b_think_ids_tp1_s${shard}.log"
  server_log="${LOG_DIR}/vllm_server_videomme_test_pnp_qwen2p5vl7b_think_ids_tp1_s${shard}.log"
  wandb_name="pnp_qwen2p5vl7b_videomme_test_think_ids_tp1_s${shard}"
  ...
  echo "[videomme] launch shard ${shard} gpu ${gpu} port ${port}"
...
echo "[videomme] waiting for all shards..."
wait
echo "[videomme] all shards finished."
